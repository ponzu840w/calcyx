...
ctest には 36 本のテストが 3 系統 (engine 28 / gui 2 / cli 6) で登録されています
...
|---|---|---|---|---|---|
| `unix` | 28 | 2 | 6 | 36 | ネイティブ Linux / macOS |
| `win` | 28 | 2 | 6 | 36 | Windows クロスビルド全テスト |
| `win-headless` | 28 | (除外) | 6 | 34 | gui ラベルを filter 除外 |
...
- CLI: C、`cli/` 以下
- TUI: FTXUI (C++17)、`tui/` 以下 — GUI と同じ `shared/sheet_model.{h,c}` を共有
...
| `./build/tui/calcyx-tui` | TUI (macOS/Linux) |
...
| `./build-win/tui/calcyx-tui.exe` | TUI (Windows) |
